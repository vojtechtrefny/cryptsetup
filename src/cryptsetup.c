...
static int _luksFormat(struct crypt_device **r_cd, char **r_password, size_t *r_passwordLen)
{
	...
	if (!ARG_SET(OPT_KEY_SIZE_ID) && !strncmp(cipher_mode, "xts-", 4)) {
		if (DEFAULT_LUKS1_KEYBITS == 128)
			ARG_SET_UINT32(OPT_KEY_SIZE_ID, 256);
		else if (DEFAULT_LUKS1_KEYBITS == 256)
			ARG_SET_UINT32(OPT_KEY_SIZE_ID, 512);
	}
#endif
	keysize = (ARG_UINT32(OPT_KEY_SIZE_ID) ?: DEFAULT_LUKS1_KEYBITS) / 8 + integrity_keysize;
...
static int token_plugin_validate_add_args(struct tools_token_handler *token_handler, void *handle, struct crypt_device *cd)
{
	if (!token_handler->create) {
		log_err(_("Plugin %s does not support token add action."), token_handler->type);
		return -ENOTSUP;
	}
	...
	return token_handler->validate_create_params(cd, handle);
}

static int token_plugin_validate_remove_args(struct tools_token_handler *token_handler, void *handle, struct crypt_device *cd)
{
	...
	if (!token_handler->validate_remove_params) {
		log_dbg("Plugin does not export validation method for token remove action.");
		return 0;
	}

	return token_handler->validate_remove_params(cd, handle);
}

static int token_plugin_add(struct crypt_device *cd, struct tools_token_handler *token_handler)
{
	int r;
	void *phandle = NULL;

	if (!token_handler || !token_handler->init || !token_handler->free)
		return -EINVAL;
	...
	r = token_plugin_validate_add_args(token_handler, phandle, cd);
	if (r)
		goto err;
...
static int token_plugin_remove(struct crypt_device *cd, struct tools_token_handler *token_handler)
{
	int r;
	void *phandle = NULL;
	...
	r = token_plugin_validate_remove_args(token_handler, phandle, cd);
	if (r)
		goto err;
...
static int token_plugin_validate_args(const char *token_action, struct tools_token_handler *token_handler)
{
	int r;
	void *phandle;
	...
	if (token_handler->init(&tool_ctx, &phandle)) {
		log_err(_("Failed to initialize plugin (%s) handle."), token_handler->type);
		return -EINVAL;
	}

	if (!strcmp(token_action, "add"))
		r = token_plugin_validate_add_args(token_handler, phandle, NULL);
	else if (!strcmp(token_action, "remove"))
		r = token_plugin_validate_remove_args(token_handler, phandle, NULL);
	else
		r = -ENOTSUP;
...
static int _token_add(struct crypt_device *cd)
{
	...
	if (token_handler.type)
		return token_plugin_add(cd, &token_handler);
...
static int _token_remove(struct crypt_device *cd)
{
	...
	if (token_handler.type)
		return token_plugin_remove(cd, &token_handler);
...
static int action_encrypt_luks2(struct crypt_device **cd)
{
	...
	*cd = NULL;
	...
	if (!ARG_SET(OPT_HEADER_ID)) {
		snprintf(header_file, sizeof(header_file), "LUKS2-temp-%s.new", ARG_STR(OPT_UUID_ID));
		...
		struct keyslot_passwords *kp, size_t kp_size)
{
	...
	if (ARG_INT32(OPT_KEY_SLOT_ID) == CRYPT_ANY_SLOT) {
		...
		snprintf(msg, sizeof(msg), _("Enter passphrase for key slot %u: "), ARG_INT32(OPT_KEY_SLOT_ID));
		 ...
		 void *data)
{
	...
	assert(!strncmp(key->longName, "plugin-", 7));

	targ = find_arg_in_args(key->longName + strlen(th->type) + 8, th->args_plugin, th->total_args_count);
	if (targ)
...
int main(int argc, const char **argv)
{
	...
	if (plugin && aname && !strcmp(aname, "token") && token_handler.type) {
		...
		tools_plugin_assign_args_to_action(&token_handler, tool_core_args,
						   ARRAY_SIZE(tool_core_args), TOKEN_ACTION);

		tool_ctx.plugin_args.name = token_handler.type;
		tool_ctx.plugin_args.args = token_handler.args_plugin;
		tool_ctx.plugin_args.count = token_handler.total_args_count;
	}
	...
	if (!strcmp(aname, TOKEN_ACTION)) {
		...
		if (!ARG_SET(OPT_PLUGIN_ID) && !ARG_SET(OPT_KEY_DESCRIPTION_ID) && !strcmp(action_argv[0], "add"))
			usage(popt_context, EXIT_FAILURE, _("Option --key-description is required."),
			      poptGetInvocationName(popt_context));

		if (!ARG_SET(OPT_PLUGIN_ID) && ARG_INT32(OPT_TOKEN_ID_ID) == CRYPT_ANY_TOKEN && !strcmp(action_argv[0], "remove"))
			usage(popt_context, EXIT_FAILURE, _("Option --token-id is required."),
			      poptGetInvocationName(popt_context));

		if (ARG_INT32(OPT_TOKEN_ID_ID) == CRYPT_ANY_TOKEN && !strcmp(action_argv[0], "export"))
			usage(popt_context, EXIT_FAILURE, _("Option --token-id is required."),
			      poptGetInvocationName(popt_context));

		/* token action specific check (plugin) */
		if (ARG_SET(OPT_PLUGIN_ID) && token_plugin_validate_args(action_argv[0], &token_handler))
			usage(popt_context, EXIT_FAILURE, _("Invalid plugin token arguments."),
			      poptGetInvocationName(popt_context));
	}
